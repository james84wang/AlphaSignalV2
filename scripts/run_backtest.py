...
Usage (from repo root):
    python scripts/run_backtest.py                     # watchlist, 5 years
    python scripts/run_backtest.py --years 3           # watchlist, 3 years
    python scripts/run_backtest.py --account 50000     # custom starting capital
    python scripts/run_backtest.py --no-save           # skip SQLite persistence

One command, per spec phase-4.md acceptance criterion.
"""
...
# Make sure the repo root is on sys.path so imports work from any cwd.
...
def _load_watchlist(cfg, sp500: bool = False) -> list[str]:
    """Return symbols from watchlist.csv only (sp500=False is the safe default)."""
    data_dir = _REPO_ROOT / "data"
    wl_path = data_dir / "watchlist.csv"

    if not wl_path.exists():
        logger.warning("No watchlist.csv found at %s — using default tickers", wl_path)
...
def _print_report(result, initial_account: float) -> None:
    m = result.metrics
    ...
    print(f"\n{'═' * 60}")
    print("  BACKTEST RESULTS — AlphaSignalV2")
    print(f"{'═' * 60}")
    ...
    print(f"  Profit Factor     : {m.get('profit_factor', 0):.2f}  (> 1 = profitable)")
    ...
    if not viable:
        print("\n  ⚠  Honest assessment: this strategy does NOT meet the spec's")
        print("     success targets with these default weights and this universe.")
        print("     Tune weights in config.yaml or expand the universe before trusting it.")

    # ── Trade log (top 10 by P&L)
    if trades:
        print(f"\n  ── Trade Log (showing {min(10, len(trades))} of {len(trades)}) ────────────────")
        print(f"  {'Symbol':<8} {'Side':<6} {'Entry':>10} {'Exit':>10} {'P&L':>9} {'%':>7} {'Reason'}")
        print("  " + sep)
        sorted_trades = sorted(trades, key=lambda t: abs(t.pnl), reverse=True)
        for t in sorted_trades[:10]:
            print(
                f"  {t.symbol:<8} {t.side:<6} {t.entry_date:>10} {t.exit_date:>10} "
                f"  {t.pnl:+9.2f} {t.pnl_pct:+7.2f}% {t.exit_reason}"
            )

    # ── Equity curve (first + last + min + max)
    ...
    print(f"  {result.params.get('earnings_blackout', '')}")
    print(f"\n{'═' * 60}\n")


def main() -> None:
    parser = argparse.ArgumentParser(description="AlphaSignalV2 Backtest Runner")
    ...
    logger.info(
        "Starting backtest: %d symbols, %s → %s, $%.0f initial",
        len(symbols), start, end, initial_account,
    )
    ...
    _print_report(result, initial_account)

    if not args.no_save and result.trades:
        cfg_hash = config_hash(_REPO_ROOT / "config.yaml")
        run_id = save_backtest_result(result, cfg_hash, duration)
